...
object CharRangeToIntrinsic : BinaryOperationIntrinsic() {
    private val charRangeClassCache = WeakHashMap<ModuleDescriptor, ClassDescriptor>()

    override fun isApplicable(name: String, first: KotlinType, second: KotlinType): Boolean =
            name == "rangeTo" && KotlinBuiltIns.isChar(first) && KotlinBuiltIns.isChar(second)
    ...
    override fun apply(context: IrTranslationContext, call: IrCall, first: JsExpression, second: JsExpression): JsExpression {
        ...
        val charRangeClass = charRangeClassCache.getOrPut(module) {
            module.findClassAcrossModuleDependencies(ClassId.topLevel(FqName("kotlin.ranges.CharRange")))
        }
        return buildJs { context.translateAsTypeReference(charRangeClass).newInstance(first, second) }
    }
}
